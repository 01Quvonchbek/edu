...
import { AppSection, Course, Achievement, ContactInfo, ContactMessage, CourseEnrollment, GithubProject } from './types';
...
const App: React.FC = () => {
  ...
  // Ma'lumotlarni localStorage'dan yuklash yoki default qiymatlarni olish
  ...
  const [teacherImage, setTeacherImage] = useState('https://images.unsplash.com/photo-1544717297-fa15739a5447?auto=format&fit=crop&q=80&w=800');
  ...
  // GitHub Repozitoriyalarini tortish funksiyasi
  const fetchGithubProjects = async () => {
    if (!contactInfo.github) return;
    setIsSyncing(true);
    setGithubError(null);
    try {
      // Username link ko'rinishida bo'lsa, uni tozalash
      const username = contactInfo.github.replace('https://github.com/', '').replace('/', '').trim();
      ...
      if (response.ok) {
        ...
        setGithubProjects([]);
        setGithubError("GitHub foydalanuvchisi topilmadi yoki xatolik yuz berdi.");
      }
    } catch (error) {
      setGithubError("Internet aloqasini tekshiring.");
  ...
  return (
    <div className="min-h-screen bg-white">
      <nav className={`fixed w-full z-50 transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-2">
            ...
            <NavLink id={AppSection.PROJECTS} label="Loyihalar" />
      ...
      {/* GitHub Loyihalar Bo'limi */}
      <section id="projects" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div className="space-y-2">
              <h2 className="text-4xl font-black text-slate-900">GitHub Loyihalar</h2>
              <p className="text-slate-600">Foydalanuvchi: <span className="font-bold text-indigo-600">@{contactInfo.github.replace('https://github.com/', '').replace('/', '')}</span></p>
            </div>
            {isSyncing && <div className="flex items-center gap-2 text-indigo-600 font-bold"><Loader2 className="animate-spin" /> Sinxronizatsiya...</div>}
          </div>
          
          {githubProjects.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {githubProjects.map((project) => (
                <a key={project.id} href={project.html_url} target="_blank" rel="noopener noreferrer" className="group bg-slate-50 border border-slate-100 p-8 rounded-[32px] hover:bg-indigo-600 hover:border-indigo-600 transition-all duration-300 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-indigo-600 group-hover:text-white group-hover:bg-white/20 transition-colors">
                      <Code2 size={24} />
                    </div>
                    <div className="flex items-center gap-1.5 text-slate-500 group-hover:text-white/80 font-bold">
                      <Star size={18} fill="currentColor" />
                      <span>{project.stargazers_count}</span>
                    </div>
                  </div>
                  <h3 className="text-xl font-bold text-slate-900 group-hover:text-white mb-2">{project.name}</h3>
                  <p className="text-slate-600 group-hover:text-white/80 text-sm mb-6 line-clamp-3 flex-grow">{project.description || "Tavsif mavjud emas."}</p>
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-200 group-hover:border-white/20">
                    <span className="text-xs font-black uppercase tracking-wider text-indigo-600 group-hover:text-white/60">{project.language || "Boshqa"}</span>
            ...
            <div className="bg-rose-50 border border-rose-100 p-12 rounded-[40px] text-center max-w-2xl mx-auto">
              <AlertCircle size={48} className="mx-auto text-rose-300 mb-4" />
              <p className="text-rose-900 font-bold text-xl">{githubError || "Loyihalar topilmadi"}</p>
              <p className="text-rose-600 mt-2">Admin panelidan GitHub foydalanuvchi nomini to'g'ri kiritganingizni tekshiring.</p>
              <button onClick={fetchGithubProjects} className="mt-6 px-6 py-3 bg-rose-600 text-white rounded-xl font-bold hover:bg-rose-700 transition">Qayta urinish</button>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {achievements.map((ach) => (
              <div key={ach.id} onClick={() => setSelectedItem({ type: 'achievement', data: ach })} className="group cursor-pointer flex space-x-4 p-8 rounded-3xl bg-white border border-slate-100 hover:bg-indigo-50 transition-all">
                <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-indigo-600 flex-shrink-0 shadow-sm"><Award size={28} /></div>
                <div>
          ...
          <div className="grid md:grid-cols-2 gap-16">
            <div className="space-y-8">
              ...
              <div className="space-y-6">
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-indigo-600"><MapPin size={24} /></div>
                  <p className="text-slate-900 font-medium">{contactInfo.address}</p>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-indigo-600"><Mail size={24} /></div>
                  <p className="text-slate-900 font-medium">{contactInfo.email}</p>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-indigo-600"><Phone size={24} /></div>
            ...
            <div className="bg-slate-50 p-8 md:p-12 rounded-[40px] shadow-sm border border-slate-100">
              <form onSubmit={handleSendMessage} className="space-y-6">
                <input name="name" type="text" required placeholder="Ismingiz" className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-indigo-500" />
                <input name="email" type="email" required placeholder="Email" className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-indigo-500" />
                <textarea name="message" required placeholder="Xabaringiz" rows={4} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-indigo-500"></textarea>
      ...
      <footer className="bg-slate-900 text-white pt-24 pb-12 text-center">
        <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-8 flex flex-col items-center">
          ...
          <div className="flex flex-wrap justify-center gap-6">
             ...
             <a href={getGithubUrl(contactInfo.github)} target="_blank" rel="noopener noreferrer" className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center hover:bg-slate-700 hover:-translate-y-1 transition-all duration-300 shadow-lg" title="GitHub">
                <Github size={28} />
             </a>
